...
def Ajouter():
    ...
    maBase = MySQLdb.connect(host="localhost", user="root", password="iai2glsi", database="gest_com")
    meConnect = maBase.cursor()


    try:
        sql = "INSERT INTO tb_achat(code,fournisseur,telephone,produit,prix,quantité) VALUES(%s,%s,%s,%s,%s,%s)"
        ...
        messagebox.showinfo("informations","Achat ajouter")
        root.destroy()
        call(["python","Achat.py"])
...
def Modifier():
    ...
    try:
        sql = "update tb_achat set fournisseur=%s,telephone=%s,produits=%s,prix=%s,quantité=%s where code= %s"
        val(fournisseur,telephone,produit,prix_achat,quantite_achte,matricule)
        meConnect.execute(sql,val)
        maBase.commit()
        derniereMatricule = meConnect.lastrowid
        messagebox.showinfo("information","achats modifier")
        root.destroy()
        call(["python","Achat.py"])
...
def Supprimer():
    ...
    maBase = MySQLdb .connect(host="localhost", user="root", password="1234", database="gest_com")
    meConnect = maBase.cursor()

    try:
        sql = "select from tb_achat where code=%s"
        val = (matricule)
        meConnect.execute(sql,val)
        maBase.commit()
        derniereMatricule = meConnect.lastrowid
...
#FOURNISSEUR
lblnumero = Label(root,text="FOURNISSEUR",font=("times new roman",15),bg="green",fg="white")
...
#retour
btnretour = Button(root,text= "RETOUR",font =("times new roman",24),bg="yellow",fg="white",command=Retour)
...
#entete
table.heading(1 , text="CODE ACHATS")
